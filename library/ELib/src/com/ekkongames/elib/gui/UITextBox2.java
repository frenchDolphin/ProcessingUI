...
public class UITextBox2 extends UIElement {
   ...
   */
  public UITextBox2(int x, int y, int xWidth, int yHeight) {
    this.x = x;
    this.y = y;
    this.elWidth = xWidth;
  ...
  void update() {
    if (mgr.app.millis() - cursorLastShowed >= CURSOR_BLINK_FREQUENCY) {
  ...
  void render() {
    if (shouldRefresh > 0) {
      ...
      if (textAlign == LEFT) {
        renderCursor(getTextWidth(text.substring(0, cursorPosition)));
      ...
      if (mgr.key != CODED) {
        ...
        if (doDefault) {
          if (!(modifiers[controlIndex] || modifiers[altIndex])) {
            erase(selectionStart, selectionEnd);
            insertAt(mgr.key + "", cursorPosition);
          } else {
  ...
  protected void insertAt(String toInsert, int index) {
    ...
    cursorPosition = index + toInsert.length();
  }
